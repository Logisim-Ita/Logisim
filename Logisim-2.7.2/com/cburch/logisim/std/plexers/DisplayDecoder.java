...
public class DisplayDecoder extends InstanceFactory {
	...
	private void updatePorts(Instance instance) {
		...
		if (dir == Direction.NORTH || dir == Direction.SOUTH) {//horizzontal
			...
			for (int i = 0; i < in; i++) {//inputs
				ps[i] = new Port(20 * i - 30, y, Port.INPUT, 1); //total lenght should be 80(10-A-20-B-20-C-20-D-10)
				ps[i].setToolTip(Strings.getter("DisplayDecoderInTip", "" + cin));
				cin++;
			}
			int j=in;
			for (int i = 0; i < out; i++) {//outputs
				ps[j] = new Port(10 * i - 30, 0, Port.OUTPUT, 1); //total lenght should be 80(10-a-10-b-10-c-10-d-10-e-10-f-10-g-10)
	...
	public void propagate(InstanceState state) {
		...
		*/
		...
		{
			decval=-1;
			if(state.getPort(0) != Value.UNKNOWN && state.getPort(1) != Value.UNKNOWN && state.getPort(2) != Value.UNKNOWN && state.getPort(3) != Value.UNKNOWN){
				for (int i = 1; i <=input; i++) {//1, 2, 3, 4
					if (state.getPort(i-1) == Value.TRUE) {//array's cell 0, 1, 2, 3
						decval=(i==4)?decval+8:(i==3)?decval+4:(i==2)?decval+2:decval+1;//for example 1101 --> 8+4+1= 13(decimal)
					}
				}
				decval++;
			}
	...
	public void paintInstance(InstancePainter painter) {
		...
		GraphicsUtil.drawCenteredText(g, Integer.toString(decval),
