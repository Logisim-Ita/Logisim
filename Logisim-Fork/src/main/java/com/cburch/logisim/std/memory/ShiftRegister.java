...
public class ShiftRegister extends InstanceFactory {
	...
	private static final int LAST_PIN = LD;
	...
	private ShiftRegisterData getData(InstanceState state) {
		...
		if (data == null) {
			data = new ShiftRegisterData(length, width);
			state.setData(data);
		} else {
			data.setDimensions(length, width);
		}
		return data;
	}
	...
	public Bounds getOffsetBounds(AttributeSet attrs) {
		...
		if (!parallel && Main.FILE_VERSION.compareTo(LogisimVersion.get(2, 15, 0, 0)) < 0) {
	...
	public void paintInstance(InstancePainter painter) {
		...
		// always print internal bits from all versions next to 2.15.0.0 mantaining
		// retro compatibility
		if (parallel || Main.FILE_VERSION.compareTo(LogisimVersion.get(2, 15, 0, 0)) >= 0) {
