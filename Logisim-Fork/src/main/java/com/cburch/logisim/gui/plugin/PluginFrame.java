...
public class PluginFrame extends LFrame {
	...
	private class MyListener implements ActionListener, LocaleListener {
		...
		public void actionPerformed(ActionEvent event) {
			...
			if (src == close) {
				WindowEvent e = new WindowEvent(PluginFrame.this, WindowEvent.WINDOW_CLOSING);
				PluginFrame.this.processWindowEvent(e);
			}
		}
	...
	private static class WindowMenuManager extends WindowMenuItemManager implements LocaleListener {
		private PluginFrame window = null;
	 ...
	 * 
	 */
	private static final long serialVersionUID = -5030986742755700957L;
	...
	private PluginFrame() {
		...
		panels = new OptionsPanel[] {new InstalledOptions(this),new StoreOptions(this),new PluginOptions(this),};
		tabbedPane = new JTabbedPane();
		int intlIndex = -1;
		for (int index = 0; index < panels.length; index++) {
			OptionsPanel panel = panels[index];
			tabbedPane.addTab(panel.getTitle(), null, panel, panel.getToolTipText());
			if (panel instanceof InstalledOptions)
				intlIndex = index;
		}
